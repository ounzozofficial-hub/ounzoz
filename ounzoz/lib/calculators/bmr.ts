 ...
 * DOM/React state. Assumes inputs already passed validateBMRInputs; it
 * still range-checks defensively and throws RangeError rather than ever
...
// Mirrors the pattern established in lib/calculators/bmi.ts: each field
// validates independently so the UI can show errors next to the field
// that caused them, per DESIGN.md Section 19.
