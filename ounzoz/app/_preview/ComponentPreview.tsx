...
const SAMPLE_BREADCRUMB_ITEMS: BreadcrumbItem[] = [
  { name: 'Home', href: '/' },
  { name: 'Sample Category', href: '/sample' },
  ...
  return (
    <div className="flex flex-col gap-[var(--space-8)] border-t-4 border-dashed border-[var(--color-brand-yellow)] bg-[var(--color-background)] py-[var(--space-8)]">
      <div className="mx-auto w-full max-w-[var(--content-max-width)] px-4 md:px-6">
        <p className="inline-block rounded-[var(--radius-full)] bg-[var(--color-brand-yellow)] px-[var(--space-4)] py-[var(--space-1)] font-[family-name:var(--font-body)] text-[var(--font-size-sm)] font-semibold text-[var(--color-brand-navy)]">
      ...
      <PreviewBlock title="Card">
        <Card>
          <p className="font-[family-name:var(--font-body)] text-[var(--font-size-base)] text-[var(--color-text-primary)]">
      ...
      <PreviewBlock title="ResultCard">
        <div className="grid grid-cols-1 gap-[var(--space-5)] md:grid-cols-3">
          <div className="flex flex-col gap-[var(--space-2)]">
            <span className="font-[family-name:var(--font-body)] text-[var(--font-size-xs)] font-semibold uppercase tracking-wide text-[var(--color-text-secondary)]">
              Empty state
            </span>
            <ResultCard
              state="empty"
      ...
      <PreviewBlock title="CalculatorLayout (full shell)">
        <div className="overflow-hidden rounded-[var(--radius-lg)] border-2 border-dashed border-[var(--color-border)]">
          <CalculatorLayout
            breadcrumbItems={SAMPLE_BREADCRUMB_ITEMS}
...
function PreviewBlock({
  ...
  return (
    <div className="mx-auto flex w-full max-w-[var(--content-max-width)] flex-col gap-[var(--space-4)] px-4 md:px-6">
      <h2 className="font-[family-name:var(--font-body)] text-[var(--font-size-sm)] font-semibold uppercase tracking-wide text-[var(--color-text-secondary)]">
        {title}
      </h2>
      {children}
    </div>
